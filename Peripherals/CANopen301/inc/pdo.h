...
struct struct_s_PDO_status {
  UNS8 transmit_type_parameter;/* 有两种用法，和PDO的传输类型的设置有关，当传输类型条目的值设置为1到240之间时该值在节点接收到同步帧后累加，
	                              相当于是一个U8类型的数据，当传输类型设置为其他值时，该值是一个只利用了最后一位的一个标志位 */
  TIMER_HANDLE event_timer;
  TIMER_HANDLE inhibit_timer;
  Message last_message;/* 在sendPdo函数中更新 */
};
...
#define TRANS_SYNC_ACYCLIC    0    /* Trans after reception of n SYNC. n = 1 to 240 *//* 同步（非周期） */
#define TRANS_SYNC_MIN        1    /* Trans after reception of n SYNC. n = 1 to 240 *//* 同步（每一个SYNC一周期） */
#define TRANS_SYNC_MAX        240  /* Trans after reception of n SYNC. n = 1 to 240 *//* 同步（每240个SYNC一周期） */
#define TRANS_RTR_SYNC        252  /* Transmission on request *//* FC *//* (仅RTR(同步)) */
#define TRANS_RTR             253  /* Transmission on request *//* FD *//* 仅RTR(事件驱动) */
#define TRANS_EVENT_SPECIFIC  254  /* Transmission on event *//* FE *//* 事件驱动（制造商指定） */
#define TRANS_EVENT_PROFILE   255  /* Transmission on event *//* FF *//* 事件驱动（设备协议和应用协议指定）*/
