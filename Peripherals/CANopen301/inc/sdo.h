...
struct struct_s_transfer {
  ...
  UNS8           shadow_of_state;/* state 的影子，只在上传和下载的启动阶段更新为state的值，完全释放线的时候清零，该变量的作用是用于判断SDO传输失败的是上传还是下载，便于重传的操作 */
  UNS8           toggle;	
  UNS32          abortCode;  /**< Sent or received */
	
  /**< index and subindex of the dictionary where to store(for a received SDO) or to read (for a transmit SDO) */
  UNS16          index;
  UNS8           subIndex;
  ...
  UNS32          objsize;           /**< Size in bytes of the object provided by data producer *//* 只应用于块传输，用于指明要传输的多个块的字节总数 */
